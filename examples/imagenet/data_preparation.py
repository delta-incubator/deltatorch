...
def extract_annotation_udf(img_path_col: pd.Series) -> pd.Series:
  def xml_to_json_annotation(img_path):
    xml_file_path = img_path.replace("dbfs:", "/dbfs").replace("Data", "Annotations").replace(".JPEG", ".xml")
    print(xml_file_path)
...
train_df = train_df.withColumn("object_id", split(element_at(split(train_df.path, "/"), -1), "_").getItem(0))
