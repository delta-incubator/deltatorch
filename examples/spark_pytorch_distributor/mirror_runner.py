...
class MirrorRunner:
    """
    MirrorRunner runs PyTorch deep learning training jobs
    ...
    in a decentralized manner. MirrorRunner can take a regular
    PyTorch program with no special distributed training code and
    ...
    def __init__(self,
        ...
        Args:
            ...
            use_custom_strategy: When true, the training function passed to the
                MirrorRunner.run method must construct and use its
                own torch.distribute.Strategy() object. When false,
                MirrorRunner constructs one for the user and wraps
                the training function in the strategy context, allowing
                the user to provide non-distributed PyTorch code that is
                executed as distributed code.
                Example with use_custom_strategy=True:
                    def train_fn():
                        import torch
                        strategy = torch.distribute.experimental \
    ...
    def run(self, train_fn, **kwargs):
        """
        Args:
            train_fn: Function that contains PyTorch training code.
                If it constructs its own torch.distribute.Strategy
                object, then construct MirrorRunner with
    ...
    @staticmethod
    def _run_pytorch_program(train_fn, use_custom_strategy, **kwargs):
    ...
    def _get_spark_task_program(self, train_fn, **kwargs):
        ...
        run_pytorch_program = MirrorRunner._run_pytorch_program
